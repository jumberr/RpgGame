...
public class AnimationBatchImport : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        var pathvalid = !string.IsNullOrEmpty(settings.path) && Directory.Exists(settings.path);
        if (!pathvalid)
        {
            GUI.color = Color.red;
        ...
        DrawUILine();
        settings.rename_anim_clips =
            EditorGUILayout.BeginToggleGroup("Rename animation clips to filename", settings.rename_anim_clips);
        {
            settings.rename_anim_clips_underscores =
                EditorGUILayout.Toggle("Spaces to underscores", settings.rename_anim_clips_underscores);
            settings.rename_anim_clips_tolower =
                EditorGUILayout.Toggle("To lowercase", settings.rename_anim_clips_tolower);
        }
        EditorGUILayout.EndToggleGroup();
        GUILayout.Space(5);

        DrawUILine();
        settings.change_loop_anim_clips =
            EditorGUILayout.BeginToggleGroup("Loop Time", settings.change_loop_anim_clips);
        {
            ...
            settings.loop_anim_clips_cycleoffset =
                EditorGUILayout.FloatField("Cycle Offset", settings.loop_anim_clips_cycleoffset);
        }
        EditorGUILayout.EndToggleGroup();
        GUILayout.Space(5);

        DrawUILine();
        settings.root_transform_rotation =
            EditorGUILayout.BeginToggleGroup("Root Transform Rotation", settings.root_transform_rotation);
        {
            settings.root_transform_rotation_bakeintopose =
                EditorGUILayout.Toggle("Bake into pose", settings.root_transform_rotation_bakeintopose);
            settings.root_transform_rotation_popupindex = EditorGUILayout.Popup("Based Upon",
                settings.root_transform_rotation_popupindex, new string[]
                {
                    "Original", "Body Orientation"
                });
            settings.root_transform_rotation_offset =
                EditorGUILayout.FloatField("Offset", settings.root_transform_rotation_offset);
        }
        EditorGUILayout.EndToggleGroup();
        GUILayout.Space(5);

        DrawUILine();
        settings.root_transform_positiony =
            EditorGUILayout.BeginToggleGroup("Root Transform Position (Y)", settings.root_transform_positiony);
        {
            settings.root_transform_positiony_bakeintopose =
                EditorGUILayout.Toggle("Bake into pose", settings.root_transform_positiony_bakeintopose);
            settings.root_transform_positiony_popupindex = EditorGUILayout.Popup("Based Upon",
                settings.root_transform_positiony_popupindex, new string[]
                {
                    "Original", "Center of Mass", "Feet"
                });
            settings.root_transform_positiony_offset =
                EditorGUILayout.FloatField("Offset", settings.root_transform_positiony_offset);
        }
        EditorGUILayout.EndToggleGroup();
        GUILayout.Space(5);

        DrawUILine();
        settings.root_transform_positionxz =
            EditorGUILayout.BeginToggleGroup("Root Transform Position (XZ)", settings.root_transform_positionxz);
        {
            settings.root_transform_positionxz_bakeintopose = EditorGUILayout.Toggle("Bake into pose",
                settings.root_transform_positionxz_bakeintopose);
            settings.root_transform_positionxz_popupindex = EditorGUILayout.Popup("Based Upon",
                settings.root_transform_positionxz_popupindex, new string[]
                {
        ...
        settings.disable_material_import =
            EditorGUILayout.Toggle("Disable Material Import", settings.disable_material_import);
        settings.mirror = EditorGUILayout.Toggle("Mirror", settings.mirror);
        settings.rig_custom_avatar =
            EditorGUILayout.ObjectField("Custom Avatar", settings.rig_custom_avatar, typeof(Avatar), false) as Avatar;
    ...
    private void ProcessDir()
    {
        ...
        if (allFiles.Count > 0)
        {
            for (int i = 0; i < allFiles.Count; i++)
            {
                ...
                EditorUtility.DisplayProgressBar($"Processing {allFiles.Count} files", filename,
                    (1f / allFiles.Count) * i);
    ...
    {
        var modelImporter = asset as ModelImporter;
    ...
    {
        editor = GetWindow<AnimationBatchImport>();
    ...
    {
        var fileInfo = Directory.GetFiles(path, "*.fbx", SearchOption.AllDirectories);
